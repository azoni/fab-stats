...
import type { TimelineResult, TimelineStats } from "./types";

const RESULTS_COL = "timeline-results";
const STATS_DOC = "timeline-stats";
const STATS_PUBLIC_COL = "timelinePlayerStats";
...
function defaultStats(): TimelineStats {
  return {
    gamesPlayed: 0,
    gamesWon: 0,
    currentStreak: 0,
...
function sanitizeStats(raw: Record<string, unknown>): TimelineStats {
  return {
...
export async function saveResult(uid: string, result: TimelineResult): Promise<void> {
  ...
  const updated: TimelineStats = {
    gamesPlayed: prev.gamesPlayed + 1,
    gamesWon: prev.gamesWon + (result.won ? 1 : 0),
    currentStreak: newStreak,
    maxStreak: Math.max(prev.maxStreak, newStreak),
    lastPlayedDate: result.date,
    totalCorrect: prev.totalCorrect + result.correctPlacements,
  ...
  logToEcosystem("minigame_completed", "Timeline completed");
}

export async function loadStats(uid: string): Promise<TimelineStats | null> {
