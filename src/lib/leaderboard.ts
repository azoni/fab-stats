...
export async function updateLeaderboardEntry(
  ...
  const entry: Omit<LeaderboardEntry, never> = {
    ...
    heroCompletionPct: overall.totalMatches > 0 ? Math.round(matches.filter((m) => m.result !== MatchResult.Bye && m.heroPlayed && m.heroPlayed !== "Unknown").length / overall.totalMatches * 100) : 0,
    opponentHeroCompletionPct: overall.totalMatches > 0 ? Math.round(matches.filter((m) => m.result !== MatchResult.Bye && m.opponentHero && m.opponentHero !== "Unknown").length / overall.totalMatches * 100) : 0,
    bothHeroesCompletionPct: overall.totalMatches > 0 ? Math.round(matches.filter((m) => m.result !== MatchResult.Bye && m.heroPlayed && m.heroPlayed !== "Unknown" && m.opponentHero && m.opponentHero !== "Unknown").length / overall.totalMatches * 100) : 0,
    topHero,
    topHeroMatches,
