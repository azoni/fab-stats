...
const STATS_DOC = "heroguesser-stats";
const STATS_PUBLIC_COL = "heroguesserPlayerStats";
...
function defaultStats(): HeroGuesserStats {
  return {
    gamesPlayed: 0,
    gamesWon: 0,
    currentStreak: 0,
...
function sanitizeStats(raw: Record<string, unknown>): HeroGuesserStats {
  return {
    ...
    guessDistribution: (raw.guessDistribution as Record<number, number>) ?? {},
  };
}

export async function saveResult(uid: string, result: HeroGuesserResult): Promise<void> {
  ...
  if (result.won) {
    dist[result.guessCount] = (dist[result.guessCount] || 0) + 1;
  }

  const updated: HeroGuesserStats = {
...
export async function loadStats(uid: string): Promise<HeroGuesserStats | null> {
  let stats: HeroGuesserStats | null = null;
