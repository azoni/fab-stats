...
const STATS_DOC = "matchupmania-stats";
const STATS_PUBLIC_COL = "matchupmaniaPlayerStats";
...
function defaultStats(): MatchupManiaStats {
  return {
    gamesPlayed: 0,
    gamesWon: 0,
    currentStreak: 0,
...
function sanitizeStats(raw: Record<string, unknown>): MatchupManiaStats {
  return {
...
export async function saveResult(uid: string, result: MatchupManiaResult): Promise<void> {
  ...
  const updated: MatchupManiaStats = {
...
export async function loadStats(uid: string): Promise<MatchupManiaStats | null> {
  let stats: MatchupManiaStats | null = null;
