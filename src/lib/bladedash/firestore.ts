...
const STATS_DOC = "bladedash-stats";
const STATS_PUBLIC_COL = "bladedashPlayerStats";
...
function defaultStats(): BladeDashStats {
  return {
    gamesPlayed: 0,
    gamesWon: 0,
    currentStreak: 0,
...
function sanitizeStats(raw: Record<string, unknown>): BladeDashStats {
  return {
    ...
    totalHintsUsed: Number(raw.totalHintsUsed) || 0,
    perfectGames: Number(raw.perfectGames) || 0,
  };
}

export async function saveResult(uid: string, result: BladeDashResult): Promise<void> {
  ...
  const isPerfect = result.won && result.hintsUsed === 0;

  const updated: BladeDashStats = {
...
export async function loadStats(uid: string): Promise<BladeDashStats | null> {
  let stats: BladeDashStats | null = null;
