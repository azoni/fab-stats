...
const STATS_DOC = "trivia-stats";
const STATS_PUBLIC_COL = "triviaPlayerStats";
...
function sanitizeStats(raw: Record<string, unknown>): TriviaStats {
  return {
...
export async function saveResult(uid: string, result: TriviaResult): Promise<void> {
...
export async function loadStats(uid: string): Promise<TriviaStats | null> {
  let stats: TriviaStats | null = null;
