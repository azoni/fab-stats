...
const STATS_DOC = "ninjacombo-stats";
const STATS_PUBLIC_COL = "ninjacomboPlayerStats";
...
function defaultStats(): NinjaComboStats {
  return {
    gamesPlayed: 0,
    gamesWon: 0,
    currentStreak: 0,
...
function sanitizeStats(raw: Record<string, unknown>): NinjaComboStats {
  return {
...
export async function saveResult(uid: string, result: NinjaComboResult): Promise<void> {
...
export async function loadStats(uid: string): Promise<NinjaComboStats | null> {
  let stats: NinjaComboStats | null = null;
