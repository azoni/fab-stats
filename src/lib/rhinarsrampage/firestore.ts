...
const STATS_DOC = "rhinarsrampage-stats";
const STATS_PUBLIC_COL = "rhinarsrampagePlayerStats";
...
function defaultStats(): RampageStats {
  return {
    gamesPlayed: 0,
    gamesWon: 0,
    currentStreak: 0,
...
function sanitizeStats(raw: Record<string, unknown>): RampageStats {
  return {
    ...
    totalBusts: Number(raw.totalBusts) || 0,
    intimidatesUsed: Number(raw.intimidatesUsed) || 0,
  };
}

export async function saveResult(uid: string, result: RampageResult): Promise<void> {
  ...
  const updated: RampageStats = {
    ...
    totalBusts: prev.totalBusts + result.busts,
    intimidatesUsed: prev.intimidatesUsed + (result.intimidateUsed ? 1 : 0),
...
export async function loadStats(uid: string): Promise<RampageStats | null> {
  let stats: RampageStats | null = null;
