...
export default function TrendsPage() {
  ...
  const filteredMatches = useMemo(() => {
    let fm = matches;
    if (filterFormat !== "all") {
      fm = fm.filter((m) => m.format === filterFormat);
    }
    ...
    if (filterTimeRange !== "all") {
      const now = new Date();
      let cutoff: Date;
      if (filterTimeRange === "30d") cutoff = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
      else if (filterTimeRange === "3m") cutoff = new Date(now.getTime() - 90 * 24 * 60 * 60 * 1000);
  ...
  const ratedComparison = useMemo(() => {
    const rated = filteredMatches.filter((m) => m.rated === true);
    const unrated = filteredMatches.filter((m) => m.rated !== true);
  ...
  // Events played
  const eventStats = useMemo(() => computeEventStats(filteredMatches), [filteredMatches]);

  // Event type stats for "Best Event Type" card
  const eventTypeStats = useMemo(() => computeEventTypeStats(filteredMatches), [filteredMatches]);
  ...
  // Unique heroes
  const uniqueHeroes = useMemo(() => new Set(filteredMatches.map((m) => m.heroPlayed)).size, [filteredMatches]);

  // Hero performance
  const heroStats = useMemo(() => computeHeroStats(filteredMatches).filter((h) => h.totalMatches >= 3), [filteredMatches]);
  ...
  const opponentMatchups = useMemo(() => {
    ...
    for (const hero of computeHeroStats(filteredMatches)) {
  ...
  const venueData = useMemo(() => {
    return computeVenueStats(filteredMatches)
  ...
  const trendData = computeTrends(filteredMatches, granularity);
  const rollingData = computeRollingWinRate(filteredMatches, windowSize);
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2 flex-wrap items-center">
        ...
        <div className="flex gap-0.5 bg-fab-surface border border-fab-border rounded-lg p-0.5">
          {([["all", "All Time"], ["6m", "6 Months"], ["3m", "3 Months"], ["30d", "30 Days"]] as const).map(([val, label]) => (
            <button
              key={val}
              onClick={() => setFilterTimeRange(val)}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <StatCard
          ...
          value={`${Math.round((filteredMatches.filter((m) => m.result === MatchResult.Win).length / filteredMatches.length) * 100)}%`}
          color={filteredMatches.filter((m) => m.result === MatchResult.Win).length / filteredMatches.length >= 0.5 ? "win" : "loss"}
        />
        <StatCard
          label="Best Day"
      ...
      {rollingData.length > 0 && (
        <CollapsibleSection id="rolling" title="Rolling Win Rate" collapsed={collapsed} toggle={toggle}>
      ...
      {/* Win Rate by Format + Event Type side by side */}
      {(formatWinRate.length > 0 || eventTypeWinRate.length > 1) && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {formatWinRate.length > 0 && (
            <CollapsibleSection id="formatWR" title="Win Rate by Format" collapsed={collapsed} toggle={toggle}>
          ...
          {eventTypeWinRate.length > 1 && (
            <CollapsibleSection id="eventTypeWR" title="Win Rate by Event Type" collapsed={collapsed} toggle={toggle}>
              <div className="space-y-3">
      ...
      {/* Hero Performance */}
      {heroStats.length > 0 && (
        <CollapsibleSection id="heroes" title="Hero Performance" subtitle="Win rate by hero played (min 3 matches)" collapsed={collapsed} toggle={toggle}>
      ...
      {(opponentMatchups.best.length > 0 || opponentMatchups.worst.length > 0) && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {opponentMatchups.worst.length > 0 && (
            <CollapsibleSection id="worstMU" title="Toughest Matchups" subtitle="Opponent heroes you struggle against (min 3 matches)" collapsed={collapsed} toggle={toggle}>
              <div className="space-y-2.5">
      ...
      {cumulativeData.length > 2 && (
        <CollapsibleSection id="cumulative" title="Cumulative Record" collapsed={collapsed} toggle={toggle}>
          <ResponsiveContainer width="100%" height={250}>
      ...
      {/* Matches Over Time */}
      <CollapsibleSection id="matchesOverTime" title="Matches Over Time" collapsed={collapsed} toggle={toggle}>
        <div className="flex justify-end mb-3">
          <div className="flex gap-1">
      ...
      {eventTypeOverTime.data.length > 1 && eventTypeOverTime.eventTypes.length > 0 && (
        <CollapsibleSection id="eventTypeOverTime" title="Event Type Performance Over Time" subtitle="Monthly win rate by event type (min 2 matches/month)" collapsed={collapsed} toggle={toggle}>
      ...
      {dayOfWeekData.length > 1 && (
        <CollapsibleSection id="dayOfWeek" title="Win Rate by Day of Week" collapsed={collapsed} toggle={toggle}>
      ...
      {/* Venue Performance */}
      {venueData.length >= 2 && (
        <CollapsibleSection id="venue" title="Venue Performance" subtitle="Win rate by location (min 3 matches)" collapsed={collapsed} toggle={toggle}>
          <div className="space-y-3">
      ...
      {playoffFinishes.length > 0 && (
        <CollapsibleSection id="finishes" title="Tournament Finishes" subtitle="Your competitive playoff placements over time" collapsed={collapsed} toggle={toggle}>
      ...
      {ratedComparison && (
        <CollapsibleSection id="rated" title="Rated vs Casual" collapsed={collapsed} toggle={toggle}>
      ...
      <CollapsibleSection id="formatPie" title="Format Breakdown" collapsed={collapsed} toggle={toggle}>
        <ResponsiveContainer width="100%" height={250}>
          <PieChart>
            <Pie
              data={pieData}
...
function CollapsibleSection({ id, title, subtitle, collapsed, toggle, children }: {
  id: string;
  title: string;
  subtitle?: string;
  ...
  return (
    <div className="bg-fab-surface border border-fab-border rounded-lg overflow-hidden">
      ...
      >
        <div>
          <h2 className="text-sm font-semibold text-fab-text text-left">{title}</h2>
          {subtitle && <p className="text-xs text-fab-dim text-left">{subtitle}</p>}
        </div>
        <svg
          className={`w-4 h-4 text-fab-muted group-hover:text-fab-text transition-transform shrink-0 ml-3 ${isCollapsed ? "" : "rotate-180"}`}
