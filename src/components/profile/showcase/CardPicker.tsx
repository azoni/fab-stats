...
const CARD_TYPES: { type: CardType; label: string; icon: string; desc: string }[] = [
  ...
  { type: "venueMastery", label: "Venue Stats", icon: "pin", desc: "Win rate by venue" },
...
export function CardPicker({ onAdd, onCancel, matches, heroStats, eventStats, opponentStats, playoffFinishes, achievements, existingCards, pointsLeft }: CardPickerProps) {
  ...
  const AUTO_TYPES = new Set<CardType>(["formatMastery", "eventTypeMastery", "venueMastery", "streakShowcase", "recentForm", "leaderboardRank"]);
  ...
  return (
    <>
      {heroStats.map((h) => {
        ...
        return (
          <button key={`${h.heroName}::${h.format}`} onClick={() => onSelect(h.heroName)} className="w-full flex items-center gap-2 p-1.5 rounded hover:bg-fab-surface-hover text-left transition-colors">
            <HeroClassIcon heroClass={info?.classes[0]} size="sm" />
            <span className="text-[10px] text-fab-text truncate flex-1">{h.heroName}{h.format ? ` (${formatShortLabel(h.format)})` : ""}</span>
