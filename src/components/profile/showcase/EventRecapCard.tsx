...
export function EventRecapCard({ event, placementType }: EventRecapCardProps) {
  ...
  return (
    <div className="spotlight-card spotlight-active bg-fab-surface border border-fab-border rounded-lg px-3 py-2 relative overflow-hidden h-full min-h-[88px]">
      <img src="/assets/cards/bg-scroll.webp" alt="" className="absolute inset-0 w-full h-full object-cover opacity-[0.15] pointer-events-none" />
      ...
      <p className="text-sm font-semibold text-fab-text truncate">{event.eventName}</p>
      <div className="flex items-center gap-2 mt-1">
        <span className="text-base font-black text-fab-text leading-none">
          {event.wins}-{event.losses}{event.draws > 0 && <span className="text-fab-dim">-{event.draws}</span>}
        </span>
        <span className={`text-xs font-semibold ${event.winRate >= 50 ? "text-fab-win" : "text-fab-loss"}`}>
