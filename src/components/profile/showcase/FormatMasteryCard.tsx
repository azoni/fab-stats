...
export function FormatMasteryCard({ matches }: FormatMasteryCardProps) {
  ...
  return (
    <div className="spotlight-card spotlight-active bg-fab-surface border border-fab-border rounded-lg px-4 py-3 relative overflow-hidden">
      <p className="text-[9px] text-fab-dim uppercase tracking-wider font-medium mb-2">Format Breakdown</p>
      <div className="space-y-1.5">
        {formats.map((f) => (
          <div key={f.format} className="flex items-center gap-2">
            <span className="text-[10px] text-fab-muted w-24 truncate shrink-0">{f.format}</span>
            <div className="flex-1 h-3 bg-fab-bg rounded-full overflow-hidden relative">
              <div
                className={`h-full rounded-full ${f.winRate >= 50 ? "bg-fab-win/30" : "bg-fab-loss/30"}`}
                style={{ width: `${(f.matches / maxMatches) * 100}%` }}
              />
              <span className="absolute inset-0 flex items-center px-1.5 text-[8px] font-medium text-fab-text">
                {f.matches}
              </span>
            </div>
            <span className={`text-[10px] font-semibold w-10 text-right ${f.winRate >= 50 ? "text-fab-win" : "text-fab-loss"}`}>
              {f.winRate.toFixed(0)}%
            </span>
          </div>
        ))}
