...
export function FormatMasteryCard({ matches, sortBy = "mostPlayed", selectedItems }: FormatMasteryCardProps) {
  ...
  return (
    <div className="spotlight-card spotlight-active bg-fab-surface border border-fab-border rounded-lg px-3 py-2 relative overflow-hidden h-full min-h-[88px]">
      <img src="/assets/cards/bg-stats.webp" alt="" className="absolute inset-0 w-full h-full object-cover opacity-[0.15] pointer-events-none" />
      ...
      <div className="space-y-1">
        {formats.map((f) => (
          <div key={f.format} className="flex items-center gap-1.5">
            <span className="text-[11px] text-fab-muted w-14 truncate shrink-0">{f.format}</span>
            <div className="flex-1 h-2.5 bg-fab-bg rounded-full overflow-hidden relative">
              ...
              <span className="absolute inset-0 flex items-center px-1 text-[9px] font-medium text-fab-text">{f.matches}</span>
            </div>
            <span className={`text-[11px] font-semibold w-8 text-right ${f.winRate >= 50 ? "text-fab-win" : "text-fab-loss"}`}>{f.winRate.toFixed(0)}%</span>
          </div>
        ))}
      </div>
      </div>
