...
export function FeedbackModal({ open, onClose }: FeedbackModalProps) {
  ...
  useEffect(() => {
    ...
    // Delay listener to avoid closing immediately from the FAB click
  ...
  return (
    <AnimatePresence>
      {open && (
        <motion.div
          ref={panelRef}
          className="fixed bottom-20 right-6 z-50 w-96 max-w-[calc(100vw-2rem)] bg-fab-surface border border-fab-border rounded-lg p-4 shadow-2xl"
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold text-fab-gold">Send Feedback</h3>
            <button
              onClick={onClose}
              className="text-fab-dim hover:text-fab-text transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="mt-4 pt-4 border-t border-fab-border">
            <p className="text-xs text-fab-dim text-center mb-2">Or reach out directly</p>
            <div className="flex items-center justify-center gap-3">
              ...
              <a
                href="https://discord.com/users/azonii"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 text-sm text-fab-muted hover:text-[#5865F2] transition-colors"
              ...
              <a
                href="https://x.com/azoniNFT"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 text-sm text-fab-muted hover:text-fab-text transition-colors"
                title="Message on X"
              >
